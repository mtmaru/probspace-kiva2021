...
def main():
    ...
    text_tokenizer = transformers.RobertaTokenizer.from_pretrained("roberta-base")
    dataset = Dataset("data/train.csv", "data/train_images", category_tokenizer, text_tokenizer)
    dataset_test = Dataset("data/test.csv", "data/test_images", category_tokenizer, text_tokenizer)
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, csv_path, image_dir_path, category_tokenizer, text_tokenizer):
    ...
    def __getitem__(self, index):
        # 画像は重いため必要になったタイミングで都度読み込む
        image_path = "{}/{}.jpg".format(self.image_dir_path, self.data.loc[index, "IMAGE_ID"])
        if pathlib.Path(image_path).exists():
            image = Image.open(image_path)
            image = self.__preprocess_normalize_image(image)
        else:
            image = torch.zeros(3, 224, 224)

        return (
            self.category_variables[index, :].to(self.device),
            self.text_variables_input_ids[index, :, :].to(self.device),
            self.text_variables_attention_mask[index, :, :].to(self.device),
    ...
    def __preprocess(self):
        ...
        # テキストを単語に分割し、単語をIDに変換
        self.text_variables_input_ids, self.text_variables_attention_mask = self.__preprocess_tokenize_text_variables(self.data, self.text_tokenizer)
    ...
    def __preprocess_tokenize_text_variables(self, data, text_tokenizer):
        # 説明の欠損を補完
        ...
        # 制御文字、文字参照、HTMLタグを削除
        data["DESCRIPTION_TRANSLATED"] = self.__preprocess_normalize_text_variable(data["DESCRIPTION_TRANSLATED"])
        data["LOAN_USE"] = self.__preprocess_normalize_text_variable(data["LOAN_USE"])

        # 単語をIDに変換、単語のマスクを作成
        text_variables_input_ids = []
        text_variables_attention_mask = []
        for column in ["DESCRIPTION_TRANSLATED", "LOAN_USE"]:
            encoded_input = text_tokenizer(data[column].fillna("").tolist(), padding = "max_length", truncation = True, max_length = 256, return_tensors = "pt")
            text_variables_input_ids.append(torch.unsqueeze(encoded_input["input_ids"], dim = 1))
            text_variables_attention_mask.append(torch.unsqueeze(encoded_input["attention_mask"], dim = 1))
        text_variables_input_ids = torch.cat(text_variables_input_ids, dim = 1)
        text_variables_attention_mask = torch.cat(text_variables_attention_mask, dim = 1)

        return text_variables_input_ids, text_variables_attention_mask
    ...
    def __preprocess_normalize_image(self, image):
...
class Estimator:
    ...
    def __init_model(self):
        ...
        for param in model.text_encoder.encoder.layer[-1].parameters():  # BERTの最終層だけfine-tuningする
            param.requires_grad = True
        for param in model.image_encoder.fc.parameters():  # ResNetの最終層だけfine-tuningする
            param.requires_grad = True
    ...
    def __init_optimizer(self, model):
        optimizer = torch.optim.Adam([
            ...
            {"params": model.image_encoder.fc.parameters(), "lr": 0.0010},
...
class Model(nn.Module):
    def __init__(self, num_embeddings, embedding_dim):
        ...
        self.image_encoder.fc = nn.Linear(in_features = 2048, out_features = 512)
        self.fc = nn.Sequential(
            nn.Linear(in_features = self.num_category_variables * embedding_dim + self.num_text_variables * 768 + self.num_tags + 512, out_features = 256),
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.Dropout(),
    ...
    def forward(self, batch):
        ...
        # (batch_size, num_text_variables, num_words)
        text_variables_input_ids = batch[1]
        # (batch_size, num_text_variables, num_words)
        text_variables_attention_mask = batch[2]
        # (batch_size, num_tags)
        tags = batch[3]
        ...
        text_variables = []
        for text_variable_i in range(text_variables_input_ids.shape[1]):
            # (batch_size, num_words, 768)
            text_variable = self.text_encoder(
                input_ids = text_variables_input_ids[:, text_variable_i, :],
                attention_mask = text_variables_attention_mask[:, text_variable_i, :]
            )["last_hidden_state"]
            # (batch_size, 1, 768)
            text_variable = text_variable[:, [0], :]  # cls
            # (num_text_variables, )
            text_variables.append(text_variable)
        # (batch_size, num_text_variables, 768)
        text_variables = torch.cat(text_variables, dim = 1)
        # (batch_size, num_text_variables * 768)
        text_variables = text_variables.view(text_variables.shape[0], text_variables.shape[1] * text_variables.shape[2])

        # (batch_size, 512)
        images = self.image_encoder(images)

        # (batch_size, num_category_variables * embedding_dim + num_text_variables * 768 + num_tags + 512)
        x = torch.cat([category_variables, text_variables, tags, images], dim = 1)
        # (batch_size, 1)
        x = self.fc(x)
        # (batch_size)
        x = torch.flatten(x)
