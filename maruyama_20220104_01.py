...
def main():
    ...
    dataset = Dataset("data/train.csv", "data/train_images", category_tokenizer)
    dataset_test = Dataset("data/test.csv", "data/test_images", category_tokenizer)
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, csv_path, image_dir_path, category_tokenizer):
...
class Model(nn.Module):
    def __init__(self, num_embeddings, embedding_dim):
        ...
        self.fc = nn.Sequential(
            nn.Linear(in_features = self.num_category_variables * embedding_dim + 768 + 768 + self.num_tags + 512, out_features = 256),
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.Dropout(),
    ...
    def forward(self, batch):
        ...
        # (batch_size, num_category_variables * embedding_dim + 768 + 768 + num_tags + 512)
        x = torch.cat([category_variables, text_dsc, text_use, tags, images], dim = 1)
        # (batch_size, 1)
        x = self.fc(x)
        # (batch_size)
        x = torch.flatten(x)
