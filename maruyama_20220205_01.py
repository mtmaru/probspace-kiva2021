...
def main():
    ...
    def print_mae_step(epoch, step, estimator, locals):
        if epoch not in history:
            history[epoch] = []
        batch_y = denormalize(locals["batch_train"][-1].cpu().detach().numpy())
        batch_y_pred = denormalize(locals["batch_train_y_pred"].cpu().detach().numpy())
    ...
    estimator = Estimator(
        ...
        embedding_dim = 8,
        num_numeric_variables = dataset.additional_data.shape[1] - 1  # -1はLOAN_IDの分
    )
    estimator = estimator.fit(dataset_train, dataset_test, [save_snapshot, print_mae_epoch], [print_mae_step])
...
class Estimator:
    ...
    def fit(self, dataset_train, dataset_test, callbacks_epoch = [], callbacks_step = []):
        ...
        for epoch in tqdm(range(self.epochs), desc = "epoch (fit)"):
            ...
            for step, batch_train in enumerate(tqdm(dataloader_train, desc = "step (fit)")):
                # https://github.com/TongtongFANG/DIW/blob/main/diw.py
                if epoch == 0:
                    batch_train_y_pred, batch_train_hidden = self.model(batch_train[:-1])
                    w = torch.ones(batch_train[-1].shape[0], device = self.device)
                else:
                    batch_test = next(iter(dataloader_test))
                    batch_train_y_pred, batch_train_hidden = self.model(batch_train[:-1])
                    batch_train_hidden_numpy = batch_train_hidden.cpu().detach().numpy()
                    batch_test_y_pred, batch_test_hidden = self.model(batch_test[:-1])
                    batch_test_hidden_numpy = batch_test_hidden.cpu().detach().numpy()
                    kernel_width = get_kernel_width(batch_train_hidden_numpy)
                    w = kmm(batch_train_hidden_numpy, batch_test_hidden_numpy, kernel_width)
                    w = torch.from_numpy(w).float().to(self.device)

                optimizer.zero_grad()
                loss = torch.mean(((batch_train_y_pred - batch_train[-1]) ** 2) * w)
                loss.backward()
                optimizer.step()
    ...
    def predict(self, dataset):
        ...
        for step, batch in enumerate(tqdm(dataloader, desc = "step (predict)")):
            with torch.no_grad():
                batch_y_pred, _ = self.model(batch[:-1])
...
class Model(nn.Module):
    def __init__(self, num_embeddings, embedding_dim, num_numeric_variables):
        ...
        self.fc = nn.Sequential(
            nn.Linear(in_features = self.num_category_variables * embedding_dim + 768 + 768 + self.num_tags + self.num_numeric_variables, out_features = 128),
            nn.BatchNorm1d(128),
            nn.ReLU(),
            nn.Dropout(),
    ...
    def forward(self, batch):
        ...
        # (batch_size, num_category_variables * embedding_dim + 768 + 768 + num_tags + num_numeric_variables)
        hidden = torch.cat([category_variables, text_dsc, text_use, tags, numeric_variables], dim = 1)
        # (batch_size, 1)
        x = self.fc(hidden)
        # (batch_size)
        x = torch.flatten(x)
    ...
    def __foward_text(self, text_variable):
        ...
        tokenized = self.text_tokenizer(text_variable, padding = True, truncation = True, max_length = 256, return_tensors = "pt")
...
# https://github.com/TongtongFANG/DIW/blob/main/kmm.py
import math
...
def kmm(x_train, x_test, sigma):
    ...
    # calculate kappa
    kappa_r = rbf_kernel(x_train, x_test, sigma)
    ...
    beta = np.asarray([i for i in beta["x"]])
