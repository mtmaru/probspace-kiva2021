...
def main():
    ...
    dataset = Dataset("data/train.csv", "data/loan_amount_local_train.csv", category_tokenizer)
    dataset_test = Dataset("data/test.csv", "data/loan_amount_local_test.csv", category_tokenizer)
...
class Model(nn.Module):
    def __init__(self, num_embeddings, embedding_dim, num_numeric_variables):
        ...
        self.fc = nn.Sequential(
            nn.Linear(in_features = self.num_category_variables * embedding_dim + 768 + 768 + self.num_tags + self.num_numeric_variables, out_features = 256),
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.Dropout(),
